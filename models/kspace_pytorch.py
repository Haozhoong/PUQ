...
def complex2ch(images, dim=1):
    # input [B, C, x, y] complex tensor
    # output [B, 2C, x, y] real tensor
    images = torch.cat([images.real, images.imag], dim=dim)
    return images

def ch2complex(images, dim=1):
    # input [B, 2C, x, y] real tensor
    # output [B, C, x, y] complex tensor
    C = int(images.shape[dim]/2)
    images = torch.complex(torch.index_select(images, dim, torch.tensor(range(C), device=images.device)),
                           torch.index_select(images, dim, torch.tensor(range(C, images.shape[dim]), device=images.device)))
    return images
...
class DataConsistencyInKspace(nn.Module):
    ...
    def perform(self, x, k0, mask):
        ...
        mask - corresponding nonzero location
        """
        k = torch.fft.fft2(ch2complex(x, dim=2), norm=self.normalized)
        out = data_consistency(k, k0, mask, self.noise_lvl)
        x_res = torch.fft.ifft2(out, norm=self.normalized)
        x_res = complex2ch(x_res, dim=2)
