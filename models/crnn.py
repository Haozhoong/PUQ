...
import numpy as np
import models.kspace_pytorch as cl
...
class CRNN_MRI(nn.Module):
    ...
    """
    def __init__(self, n_ch=2, nf=64, ks=3, nc=5, nd=5, dropout_rate=0):
        """
        ...
        drops = []
        for i in range(nc):
            drops.append(nn.Dropout2d(dropout_rate))
            
        self.dropout_flag = dropout_rate > 0
        self.drops = drops
        self.dcs = dcs
    ...
    def forward(self, x, k, m, test=False):
        ...
        """
        net = {}
        n_seq, n_batch, n_ch, width, height = x.size()
        size_h = [n_seq*n_batch, self.nf, width, height]
        if test:
            with torch.no_grad():
                hid_init = Variable(torch.zeros(size_h)).to(x.device)
        else:
            hid_init = Variable(torch.zeros(size_h)).to(x.device)
        ...
        for i in range(1,self.nc+1):
            ...
            if self.dropout_flag:
                net['t%d_x2'%i] = self.drops[i](net['t%d_x2'%i])
